...
while(cont <= length(diffDiagn1)){
  ...
  pl3 <- ggplot(pcbilTucsLong, aes(score_cats, value, fill= factor(cluster, levels = c("extrahepatic-pcbil-tuc","intrahepatic-ccc-tuc","intestinal-tuc","hcc-tuc") ))) + geom_bar(stat="identity", position="dodge", alpha = 0.85) + facet_wrap( ~ marker, ncol=ncols[cont]) + xlab("Positive* immunoreactivity") + ylab("% cases")  + scale_fill_manual("Tumour types",values = compBarCol[[cont]])
